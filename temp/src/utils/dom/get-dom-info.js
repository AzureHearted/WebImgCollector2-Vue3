// 获取Dom信息
export default async function getDOMInfo(dom, type, name) {
    let result = "";
    switch (type) {
        case "value":
            // 处理值的情况
...
function getDOMValue(dom) {
    // 判断DOM类型
    return valueElements.includes(dom.constructor)
        ? dom.value
        : "";
}
// 获取元素的attribute属性值
function getDOMAttribute(dom, _name) {
    let value = "";
    // 拆分name判断是否有多个值
...
// 获取元素的property属性值
function getDOMProperty(dom, _name) {
    let value = "";
    // 拆分name判断是否有多个值
    const names = _name.split("|");
    // 对每个值进行匹配
