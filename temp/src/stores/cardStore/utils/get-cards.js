...
// 当获取到所有DOM时的回调
onAllDOMGet = async (doms) => doms, // 用于给用户过滤dom
// 每当有一个卡片获取到的时候的回调
onCardGet = async () => { }) {
    // 卡片列表
    const cardList = [];
    ...
    if (rule.region.enable) {
        ...
        for (let i = 0; i < regionDOMs.length; i++) {
            ...
            // s source的匹配
            const source = await handleRegionGetInfo(rule.source, regionDOM, async (value, dom) => {
                dom = dom || regionDOM;
                // 元信息获取
            ...
            if (rule.preview.enable) {
                preview = await handleRegionGetInfo(rule.preview, regionDOM, async (value, dom) => {
                    // 如果sourceDOM不存在，则使用当前区域DOM作为sourceDOM。
            ...
            if (rule.description.enable) {
                description = await handleRegionGetInfo(rule.preview, regionDOM, async (value, dom) => {
                    // 如果sourceDOM不存在，则使用当前区域DOM作为sourceDOM。
        ...
        // 获取所有 previewDOMs
        let previewDOMs = (rule.preview.enable
            ? getDOM(rule.preview.selector, {
                mode: "all",
            })
            : sourceDOMs);
        // 获取所有 descriptionDOMs
        let descriptionDOMs = (rule.description.enable
            ? getDOM(rule.description.selector, {
                mode: "all",
            })
            : sourceDOMs);
        // 将所有DOMs长度统一成sourceDOMs的长度，因为它们应该是一一对应的。
        const maxLength = Math.max(sourceDOMs.length, previewDOMs.length, descriptionDOMs.length);
        ...
        for (let i = 0; i < sourceDOMs.length; i++) {
            ...
            // s 直接获取source信息
            const source = {
                url: await getDOMInfo(sourceDOM, rule.source.infoType, rule.source.name),
                dom: sourceDOM,
                meta: { valid: false, width: 0, height: 0 }, // 初始化meta未一个无效值
            };
            // 获取source.meta
            // 先使用dom进行判断
            source.meta = await getMeta(source.dom);
            ...
            if (rule.preview.enable) {
                ...
                // 获取preview.meta
                // 先使用dom进行判断
                preview.meta = await getMeta(preview.dom);
            ...
            if (rule.description.enable) {
                ...
                description = {
                    title: await getDOMInfo(descriptionDOM, rule.description.infoType, rule.description.name),
                    dom: descriptionDOM,
                };
            }
            else {
...
// 获取在region模式下信息的处理函数
async function handleRegionGetInfo(rule, // 规则对象
regionDOM = document.body, //区域DOM
callback) {
    // 获取选择器
...
function fillArrayToLength(array, length, value) {
...
async function getMeta(target, method = "auto") {
    let meta = {
        valid: false,
        width: 0,
        height: 0,
...
// 获取元信息(通过Blob对象)
async function getMetaByBlob(blob) {
    let meta;
    // 先推断类型
...
export function getImgMetaByImage(url) {
...
export function getImgMetaByBlob(blob) {
    let meta;
    return new Promise((resolve) => {
...
// 获取DOM的natural尺寸
function getDOMNaturalSize(dom) {
...
function inferUrlType(url) {
    ...
    const videoRegex = /(mp4|avi|mov|mkv|mpeg|mpg|wmv|3gp|flv|f4v|rmvb|webm|ts|webp|ogv)/i;
...
// 推测Blob类型
function inferBlobType(blob) {
    const [mainType, subType] = blob.type.split("/");
    return {
        mainType,
        subType,
    };
