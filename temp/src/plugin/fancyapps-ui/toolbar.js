export default {
    ...
    items: {
      ...
      `,
            // 点击事件定义
            click: (instance) => {
                const userSlides = instance.instance.userSlides;
                let index = 0;
                if (userSlides.length > 1) {
                    index = Number(instance.instance.container?.querySelector(".is-nav-selected")?.dataset.index);
                }
      ...
      `,
            click(instance) {
                ...
                instance.instance.close(); // 关闭当前图片，但不关闭弹窗。
                setTimeout(() => {
                    userSlides[index].thumbEl.scrollIntoView({
                        behavior: "smooth",
                        block: "center",
                        inline: "center",
                    });
